...
/**
 * Script to generate accurate previews for ALL two-column ATS templates
 * This ensures the preview images show the correct two-column layout with sidebar
 */
  ...
  projects: [
    {
      ...
      description: "Built a scalable e-commerce platform using React and Node.js. Implemented payment processing, inventory management, and user authentication.",
      technologies: ["React", "Node.js", "MongoDB", "Stripe"]
    }
  ]
};

// All two-column ATS template configurations
const twoColumnAtsTemplates = {
...
function generateAccurateResumeHTML(templateKey, template) {
  ...
  // Generate layout - ALWAYS two-column for these templates
  ...
  const layoutHTML = `<div style="width:100%;padding:2rem;background-color:${styles.colors.background};font-family:${styles.fontFamily};color:${styles.colors.text};line-height:${styles.lineHeight};font-size:${styles.fontSize};">
    ${header}
...
function generateAccurateHeader(headerStyle, data, styles) {
  ...
  // Default header for standard, professional, tech, marketing, sales, hr-specialist, creative-designer
  return `<header style="margin-bottom:1.5rem;border-bottom:2px solid #d1d5db;padding-bottom:1rem;text-align:center;">
    <h1 style="font-size:1.875rem;font-weight:700;margin-bottom:0.5rem;color:${styles.colors.primary};font-family:${styles.fontFamily};line-height:1.1;">${data.name}</h1>
    <p style="font-size:1.125rem;margin-bottom:0.75rem;color:${styles.colors.secondary};font-family:${styles.fontFamily};line-height:1.1;">${data.jobTitle}</p>
    <div style="display:flex;justify-content:center;align-items:center;gap:1rem;font-size:0.875rem;color:${styles.colors.accent};font-family:${styles.fontFamily};">
        ...
        <ul style="list-style:none;margin:0;padding:0;font-family:${styles.fontFamily};">
          ${data.skills.map(skill => `
            <li style="display:flex;align-items:center;margin-bottom:0.25rem;">
              <span style="color:${styles.colors.accent};font-size:0.75rem;margin-right:0.5rem;">•</span>
              <span style="color:${styles.colors.text};font-size:0.875rem;font-family:${styles.fontFamily};line-height:1.4;">${skill.name}</span>
            </li>
          `).join('')}
        </ul>
      </section>`;
...
async function generateAllTwoColumnAtsPreviews() {
  console.log('🚀 Generating Accurate Two-Column ATS Template Previews...\n');
  console.log('📋 This script will generate previews that match exactly what customers see\n');
  console.log('🎯 Focusing on ALL two-column ATS templates to fix layout mismatches\n');
  ...
  try {
    ...
    for (const [templateKey, template] of Object.entries(twoColumnAtsTemplates)) {
      try {
        console.log(`🔄 Generating accurate two-column preview for: ${template.name}...`);
    ...
    console.log('\n🎉 Accurate Two-Column Preview Generation Complete!');
    console.log(`✅ Successfully generated: ${successCount} accurate two-column previews`);
    if (errorCount > 0) {
      console.log(`❌ Failed generations: ${errorCount} templates`);
    }
    console.log(`📁 PNG files saved in: ${previewsDir}`);
    console.log('\n💡 These previews should now show the correct two-column layout with sidebar!');
    console.log('\n🔧 Next steps:');
    console.log('   1. Test the templates in the resume builder');
    console.log('   2. Verify the preview images show two-column layout');
    console.log('   3. Check that sidebar sections are visible in previews');
...
// Run the generation
generateAllTwoColumnAtsPreviews().catch(console.error);
