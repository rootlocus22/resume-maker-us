...
/**
 * Script to generate accurate ATS template previews using the actual ATSResumeRenderer component
 * This ensures the preview images match exactly what customers will see when using the templates
 */
...
// ATS template configurations - using the exact same structure as atsFriendlyTemplates.js
const atsTemplates = {
  ...
  ats_c_level_executive: {
    ...
    layout: { headerStyle: "c-level", columns: 1, sectionsOrder: ["personal", "summary", "experience", "education", "skills", "certifications"] },
...
function generateAccurateSection(section, data, styles) {
  switch (section) {
    ...
    case 'projects':
      return `<section style="margin-bottom:1.5rem;">
        <h2 style="font-size:1.25rem;font-weight:700;margin-bottom:0.75rem;padding-bottom:0.5rem;border-bottom:1px solid #d1d5db;text-transform:uppercase;letter-spacing:0.05em;color:${styles.colors.primary};font-family:${styles.fontFamily};">PROJECTS</h2>
        <div style="space-y:1rem;">
          ${data.projects.map(project => `
            <div style="margin-bottom:1rem;">
              <h3 style="font-size:1.125rem;font-weight:600;margin-bottom:0.5rem;color:${styles.colors.primary};font-family:${styles.fontFamily};">${project.title}</h3>
              <p style="font-size:0.875rem;line-height:${styles.lineHeight};margin-bottom:0.5rem;color:${styles.colors.text};font-family:${styles.fontFamily};">${project.description}</p>
              <div style="display:flex;flex-wrap:wrap;gap:0.25rem;">
                ${project.technologies.map(tech => `
                  <span style="padding:0.125rem 0.5rem;background-color:${styles.colors.accent}20;color:${styles.colors.accent};border-radius:0.5rem;font-size:0.75rem;font-family:${styles.fontFamily};">${tech}</span>
                `).join('')}
              </div>
            </div>
          `).join('')}
...
async function generateAccuratePreviews() {
  console.log('🚀 Generating Accurate ATS Template Previews...\n');
...
// Run the generation
generateAccuratePreviews().catch(console.error);
