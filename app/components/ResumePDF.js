...
function renderDescriptionBulletsPDF(description) {
  ...
  if (bulletLines.length >= Math.max(2, lines.length / 2)) {
    return (
      <View style={{ marginLeft: 12, marginTop: 2 }}>
        {lines.map((line, idx) => (
          <Text key={idx} style={{ fontSize: 11, lineHeight: 1.5 }}>
            • {line.replace(/^[-•*]\s*/, "")}
          </Text>
        ))}
      </View>
  ...
  return lines.map((line, idx) => (
    <Text key={idx} style={{ fontSize: 11, lineHeight: 1.5 }}>{line}</Text>
  ));
}

export const ResumePDF = ({ data, template = "default", customColors = {} }) => {
      ...
      switch (section) {
          ...
          return (
            <View key={section} style={[styles.section, sectionStyle]} wrap={false}>
              <View style={styles.sectionTitle}>
                {layout.showIcons && <Text>{iconMap.Bookmark}</Text>}
        ...
        case "experience":
          return (
            <View key={section} style={[styles.section, sectionStyle]} wrap={false}>
              <View style={styles.sectionTitle}>
                {layout.showIcons && <Text>{iconMap.Briefcase}</Text>}
              ...
              {data.experience.map((exp, index) => (
                <View key={index} style={styles.experienceItem}>
                  ...
                  {renderDescriptionBulletsPDF(exp.description)}
                </View>
              ))}
            </View>
          );
        ...
        case "education":
          return (
            <View key={section} style={[styles.section, sectionStyle]} wrap={false}>
              <View style={styles.sectionTitle}>
                {layout.showIcons && <Text>{iconMap.GraduationCap}</Text>}
              ...
              {data.education.map((edu, index) => (
                <View key={index} style={{ marginBottom: 8 }}>
                  ...
                  {renderDescriptionBulletsPDF(edu.description)}
                </View>
              ))}
            </View>
          );
        ...
        case "skills":
          return (
            <View key={section} style={[styles.section, sectionStyle]} wrap={false}>
              <View style={styles.sectionTitle}>
                {layout.showIcons && <Text>{iconMap.Wrench}</Text>}
        ...
        case "certifications":
          return (
            <View key={section} style={[styles.section, sectionStyle]} wrap={false}>
              <View style={styles.sectionTitle}>
                {layout.showIcons && <Text>{iconMap.Award}</Text>}
        ...
        case "languages":
          return (
            <View key={section} style={[styles.section, sectionStyle]} wrap={false}>
              <View style={styles.sectionTitle}>
                {layout.showIcons && <Text>{iconMap.Languages}</Text>}
