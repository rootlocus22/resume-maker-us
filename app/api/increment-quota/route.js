...
export async function POST(request) {
  try {
    ...
    console.log(`🔄 Incrementing ${quotaType} quota for user: ${userId} by ${amount}`);
    
    // Check if user is a team member - skip quota increment
    ...
    if (userDoc.exists) {
      ...
      if (isTeamMember) {
        console.log(`👥 User ${userId} is a team member - skipping quota increment`);
    ...
    if (updatedDoc.exists) {
      ...
      console.log(`✅ Quota incremented successfully. New ${quotaType} count:`, updatedData.usage[quotaType]);
    } else {
